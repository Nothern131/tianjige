...
(function (global) {
  ...
  // 天干对应六兽起始
  var GAN_LIUSHOU = { 甲: '青龙', 乙: '青龙', 丙: '朱雀', 丁: '朱雀', 戊: '勾陈', 己: '螣蛇', 庚: '白虎', 辛: '白虎', 壬: '玄武', 癸: '玄武' };
  ...
  function _wxRelation(wo, ta) {
    ...
    if ((wo === '木' && ta === '水') || (wo === '火' && ta === '木') ||
        (wo === '土' && ta === '火') || (wo === '金' && ta === '土') ||
        (wo === '水' && ta === '金')) return '父母';
    // 我生者子孙：木生火、火生土、土生金、金生水、水生木
    if ((wo === '木' && ta === '火') || (wo === '火' && ta === '土') ||
        (wo === '土' && ta === '金') || (wo === '金' && ta === '水') ||
        (wo === '水' && ta === '木')) return '子孙';
    // 克我者官鬼：金克木、水克火、木克土、火克金、土克水
    if ((wo === '木' && ta === '金') || (wo === '火' && ta === '水') ||
        (wo === '土' && ta === '木') || (wo === '金' && ta === '火') ||
        (wo === '水' && ta === '土')) return '官鬼';
    // 我克者妻财
    return '妻财';
  ...
  function generateInterpretation(gua, dongYao, shiYao, yingYao, liuQin, liuShou, changedGua) {
    ...
    parts.push('上卦' + gua.upper + '为' + BA_GUA[gua.upper].xiang + '，下卦' + gua.lower + '为' + BA_GUA[gua.lower].xiang + '。');
  ...
  function _getJudgment(gua, dongYao, shiYao, liuQin, changedGua, liuShou, yingYao) {
    ...
    var shiJ = shiJudgments[shiQin] || { yang: '世爻临' + shiQin, yin: '需留意相关风险', transform: '趋吉避凶，顺势而为', timing: '宜守正待时' };
    ...
    if (yingQin) {
      ...
      if (shiQin === yingQin) {
        parts.push('世应同临' + shiQin + '——内外一致，自身与外部环境同频共振。吉面：目标明确，内外统一；凶面：易陷入单一视角，需警惕"同声相应"带来的盲区。');
      } else if ((shiQin === '妻财' && yingQin === '官鬼') || (shiQin === '官鬼' && yingQin === '妻财')) {
        parts.push(shiYingPair + '——财官相生，事业与财富相互促进。吉面：以事业带财富或以财富助事业；凶面：需防"财官两旺"导致的精力分散。');
      } else if ((shiQin === '子孙' && yingQin === '官鬼') || (shiQin === '官鬼' && yingQin === '子孙')) {
        parts.push(shiYingPair + '——【冲突信号】福神与官鬼相克，事业压力与逍遥心态矛盾。吉面：在压力中保持轻松心态；凶面：过于放松可能错失良机，过于紧张则身心俱疲。');
      } else if ((shiQin === '兄弟' && yingQin === '妻财') || (shiQin === '妻财' && yingQin === '兄弟')) {
        parts.push(shiYingPair + '——【冲突信号】兄爻克财，竞争与财富并存。吉面：合作可分利共赢；凶面：需防因利益分配或朋友借贷导致破财。');
      } else if ((shiQin === '父母' && yingQin === '子孙') || (shiQin === '子孙' && yingQin === '父母')) {
        parts.push(shiYingPair + '——【冲突信号】文书之劳与逍遥之心矛盾。吉面：在规则中保持灵活；凶面：思虑过多阻碍行动，或行动过多忽视规划。');
      } else {
    ...
    if (dongCount === 1) {
      ...
      parts.push('一爻独动（第' + dyPos + '爻，临' + dyQin + (dyShou ? '·' + dyShou : '') + '），事态明朗，此爻为全局关键。');

      // 动爻与世爻的交叉分析
      if (dyPos === shiYao) {
        parts.push('动爻即为世爻——变化由您自身引发，主动权在您手中。吉面：可控可调；凶面：成败皆系于己，责任重大。');
      } else if (dyPos === yingYao) {
        parts.push('动爻即为应爻——变化来自外部环境或对方，非您所能完全控制。吉面：顺势而为；凶面：被动应对，需灵活应变。');
      }

      // 动爻六亲与世爻六亲的交叉
      if (dyQin === '妻财' && shiQin === '兄弟') {
        parts.push('【核心冲突】财爻动而世临兄弟——求财之心与竞争之实并存。趋吉：合作分利而非独占，选择可靠的合伙人；避凶：避免因利益分配不均而引发矛盾，谨慎为他人担保。');
      } else if (dyQin === '官鬼' && shiQin === '子孙') {
        parts.push('【核心冲突】官鬼动而世临子孙——事业压力与安逸心态冲突。趋吉：在轻松中保持进取，利用创造力化解压力；避凶：避免因过于放松而错失良机。');
      } else if (dyQin === '子孙' && shiQin === '官鬼') {
        parts.push('【核心冲突】子孙动而世临官鬼——福神解忧之力化解事业压力。趋吉：利用创意和轻松心态化解职场难题；避凶：不要因"解忧"而完全放弃责任。');
      } else if (dyQin === '父母' && shiQin === '子孙') {
        parts.push('【核心冲突】父爻动而世临子孙——文书之劳与逍遥之心矛盾。趋吉：重要文书之事委托他人或设定明确截止日期；避凶：避免因拖延而导致文书纠纷。');
      }
      ...
      // 二爻交叉分析
      var dy1 = dongYao[0], dy2 = dongYao[1];
      var q1 = liuQin[dy1 - 1], q2 = liuQin[dy2 - 1];
      if ((q1 === '妻财' && q2 === '官鬼') || (q1 === '官鬼' && q2 === '妻财')) {
        parts.push('财官二爻齐动——事业与财富双重变化。趋吉：财官相生，事业带财或财助事业；避凶：精力分散，需明确主次。');
      } else if ((q1 === '子孙' && q2 === '官鬼') || (q1 === '官鬼' && q2 === '子孙')) {
        parts.push('【冲突信号】子孙与官鬼二爻齐动——福祸相依，矛盾交织。趋吉：在变化中找到平衡点；避凶：避免在两个极端之间摇摆不定。');
      }
    } else if (dongCount >= 3) {
    ...
    if (changedGua) {
      ...
      if (gua.name === changedGua.name) {
        parts.push('本卦与之卦相同——事态虽有变动但终归原位。吉面：方向正确，坚持即可；凶面：可能原地踏步，需审视是否方法有误。');
      }
    }
  ...
  function _quickGuaDesc(name) {
    var map = {
      '乾为天': '顺利通达', '坤为地': '包容顺遂', '水雷屯': '艰难初创', '山水蒙': '启蒙待发',
      '水天需': '等待时机', '天水讼': '有争讼', '地水师': '竞争激烈', '水地比': '亲和团结',
      '风天小畜': '小有成效', '天泽履': '谨慎前行', '地天泰': '亨通顺利', '天地否': '阻滞不通',
      '天火同人': '志同道合', '火天大有': '大获丰收', '地山谦': '谦逊受益', '雷地豫': '愉悦顺利',
      '泽雷随': '顺势而为', '山风蛊': '整顿革新', '地泽临': '亲临其事', '风地观': '观察等待',
      '火雷噬嗑': '突破阻碍', '山火贲': '外华内实', '山地剥': '衰败止损', '地雷复': '转机重现',
      '天雷无妄': '守正得吉', '山天大畜': '厚积薄发', '山雷颐': '养精蓄锐', '泽风大过': '纠偏归正',
      '坎为水': '有险阻', '离为火': '光明依附', '泽山咸': '感应和合', '雷风恒': '持之以恒',
      '天山遁': '退避得吉', '雷天大壮': '强盛得势', '火地晋': '步步高升', '地火明夷': '韬光养晦',
      '风火家人': '家和事兴', '火泽睽': '求同存异', '水山蹇': '艰难曲折', '雷水解': '困难化解',
      '山泽损': '损小利大', '风雷益': '利人利己', '泽天夬': '果断决策', '天风姤': '意外遇合',
      '泽地萃': '汇聚力量', '地风升': '逐步上升', '泽水困': '困厄待解', '水风井': '养人利物',
      '泽火革': '变革图新', '火风鼎': '革故鼎新', '震为雷': '震动不安', '艮为山': '适可而止',
      '风山渐': '循序渐进', '雷泽归妹': '慎重抉择', '雷火丰': '丰盛防衰', '火山旅': '谨慎行事',
      '巽为风': '顺势而为', '兑为泽': '喜悦和合', '风水涣': '凝聚人心', '水泽节': '节制有度',
      '风泽中孚': '诚信得吉', '雷山小过': '小处谨慎', '水火既济': '守成防变', '火水未济': '继续努力'
    };
  ...
  function _getAdvice(gua, dongYao, shiYao, liuQin, liuShou, yingYao, changedGua) {
    ...
    var sa = shiAdvices[shiQin] || ['趋吉：顺势而为，把握机遇。', '避凶：谨慎行事，避免风险。', '关键：趋吉避凶，天道酬勤。'];
    advices.push(sa[0]);
    advices.push(sa[1]);
    advices.push(sa[2]);
    ...
    if (yingQin && yingQin !== shiQin) {
      ...
      if ((shiQin === '妻财' && yingQin === '兄弟') || (shiQin === '兄弟' && yingQin === '妻财')) {
        advices.push('世应财兄相克——核心矛盾在"钱"与"人"。建议：① 合作时明确权责利，书面约定；② 不因朋友关系而放松财务纪律；③ 合理分配利益，避免"兄弟反目"。');
      } else if ((shiQin === '官鬼' && yingQin === '子孙') || (shiQin === '子孙' && yingQin === '官鬼')) {
        advices.push('世应官子相克——核心矛盾在"压力"与"自由"。建议：① 在压力中保持轻松心态，不被焦虑控制；② 过于安逸时主动给自己设定挑战；③ 找到事业与生活的平衡点。');
      } else if ((shiQin === '父母' && yingQin === '妻财') || (shiQin === '妻财' && yingQin === '父母')) {
        advices.push('世应父财相克——核心矛盾在"文书/知识"与"财富"。建议：① 以知识生财，将专业技能转化为收入；② 投资自己（学习）是回报率最高的投资；③ 避免因追求财富而忽视文书和法律风险。');
      }
    }
    ...
    if (dongCount === 1) {
      ...
      advices.push('二爻齐动——两件事同时在变化，建议：① 分清主次，先解决最关键的动爻问题；② 关注两爻之间的生克关系，尽量让它们"相生"而非"相克"。');
    } else if (dongCount >= 3) {
      advices.push('多爻齐动，局势复杂。建议：① 不宜同时推进多个方向，聚焦一处；② 以静制动，观察变化后再决策；③ 重大事项暂缓，待局势明朗。');
    } else {
      advices.push('静卦策略——当前宜静不宜动。守正待时，积蓄力量，等待有利时机再行动。');
      advices.push('静卦不等于"什么都不做"，而是"做该做的事，不做冒险的事"。日常积累、学习、维护关系都是静卦中的"正事"。');
    }
    ...
    if (changedGua && changedGua.name !== gua.name) {
      advices.push('');
      advices.push('【变卦应对】');
      advices.push('之卦「' + changedGua.name + '」提示最终方向。建议提前了解' + changedGua.name + '卦德，为最终结果做好准备。');
      var changedAdvice = {
    ...
    // ===== 通用心法 =====
    advices.push('');
    advices.push('【心法】占卜为镜，照见的是当下的趋势与可能。吉非纯吉，凶非纯凶。最终的决策权在自己手中——天道酬勤，人事为本，卦象为参考而非宿命。');
