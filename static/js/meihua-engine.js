...
(function (global) {
  ...
  function calcShengKe(tiWuxing, yongWuxing) {
    ...
    if (tiWuxing === yongWuxing) {
      ...
      level = 3;
      relation = '体用比和，中吉';
      desc = '体卦' + tiWuxing + '与用卦' + yongWuxing + '五行相同，比和之象。双方力量均衡，相互扶持，主事可成，但需主动争取。';
      duality = {
      ...
      level = 4;
      relation = '用生体，大吉';
      desc = '用卦' + yongWuxing + '生体卦' + tiWuxing + '，用卦之气滋养体卦。凡事顺遂，他人相助，事半功倍，求谋有成，财利可期。此为上吉之象。';
      duality = {
      ...
      level = 2;
      relation = '体生用，小吉';
      desc = '体卦' + tiWuxing + '生用卦' + yongWuxing + '，体卦之气泄于用卦。虽有付出，但可积德结缘，长远有利。宜主动付出，不宜急求回报。';
      duality = {
      ...
      level = 0;
      relation = '用克体，大凶';
      desc = '用卦' + yongWuxing + '克体卦' + tiWuxing + '，用卦克制体卦。诸事不顺，外力压制，宜守不宜攻，静待时机。行事需格外谨慎，避免正面冲突。';
      duality = {
      ...
      level = 1;
      relation = '体克用，凶';
      desc = '体卦' + tiWuxing + '克用卦' + yongWuxing + '，体卦克制用卦。虽能成事，但耗费心力，劳而有获。宜量力而行，不宜过度消耗。';
      duality = {
      ...
      duality = { yang: '保持信心，积极面对', yin: '需谨慎行事，不可大意', transform: '综合判断，多方考量', timing: '宜守正待时' };
    }
  ...
  function generateInterpretation(original, hu, changed, shengKe, tiGua, yongGua) {
    ...
    if (origInterp) {
      ...
      lines.push('上卦' + original.shang_gua + '为' + BAGUA[BAGUA_NAME_TO_IDX[original.shang_gua]].wuxing +
        '，下卦' + original.xia_gua + '为' + BAGUA[BAGUA_NAME_TO_IDX[original.xia_gua]].wuxing + '。');
    }
  ...
  function analyzeThreeGuaPath(original, hu, changed, tiGua, yongGua) {
    ...
    // 本卦→互卦：当前状态的内在变化
    if (origName === huName) {
      parts.push('本卦与互卦相同——当前状态的内在变化与外表一致，表里如一。吉面：方向明确，没有隐藏矛盾；凶面：缺乏变化的内在动力，可能陷入惯性。');
    } else {
    ...
    // 本卦→变卦：起点到终点
    if (origName === chgName) {
      parts.push('本卦与变卦相同——事态虽有波折但终归原位。吉面：初心不改，方向正确；凶面：可能原地踏步，需审视方法是否需要调整。');
    }
