...
function renderZiweiResult(container, result, question) {
  ...
  var gongNames = ZiWeiEngine.GONG_NAMES || ['\u547D\u5BAB','\u5144\u5F1F','\u592B\u59BB','\u5B50\u5973','\u8D22\u5E1B','\u75BE\u5384','\u8FC1\u79FB','\u4EA4\u53CB','\u5B98\u797F','\u7530\u5B85','\u798F\u5FB7','\u7236\u6BCD'];
  ...
  for (var i = 0; i < gongs.length; i++) {
    ...
    gongCards += '\
      <div class="ziwei-gong-card ' + isMing + ' ' + isShen + '">\
        <div class="gong-header">\
          <span class="gong-name">' + g.name + marker + '</span>\
          <span class="gong-ganzhi">' + g.ganZhi + '</span>\
        </div>\
        <div class="gong-stars">' + starStr + '</div>\
      </div>\
    ';
  }
  ...
  var html = '\
    <div class="section-header">\
      ...
      <p class="page-subtitle">\u519C\u5386' + (result['\u519C\u5386\u5E74'] || '?') + '\u5E74' + (result['\u519C\u5386\u6708'] || '?') + '\u6708' + (result['\u519C\u5386\u65E5'] || '?') + '\u65E5 | \u5E74\u5E72' + (result['\u5E74\u5E72'] || '?') + ' | \u547D\u5BAB\u4E3B\u661F\uFF1A' + mingGongZhuXing + '</p>\
    </div>\
\
    <div class="glass-card mb-24">\
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px;flex-wrap:wrap;">\
        <span style="font-size:1.2rem;font-weight:700;color:var(--gold);">\u547D\u5BAB\uFF1A' + mingGong + '</span>\
        <span style="color:var(--text-secondary);">\u8EAB\u5BAB\uFF1A' + shenGong + '</span>\
        <span style="color:var(--text-secondary);">' + wuxingJu + '</span>\
      </div>\
\
      <div style="margin-bottom:16px;color:var(--text-secondary);">\
        \u56DB\u5316\uFF1A' + (siHua.lu || '?') + '\u5316\u7984 \u00B7 ' + (siHua.quan || '?') + '\u5316\u6743 \u00B7 ' + (siHua.ke || '?') + '\u5316\u79D1 \u00B7 ' + (siHua.ji || '?') + '\u5316\u5FCC\
      </div>\
\
      <div style="margin-bottom:16px;padding:12px;background:rgba(184,154,92,0.05);border-radius:8px;border-left:3px solid var(--gold);">\
        <strong style="color:var(--gold);">\u603B\u4F53\u8FD0\u52BF\uFF1A</strong> ' + overallVerdict + '\
      </div>\
\
