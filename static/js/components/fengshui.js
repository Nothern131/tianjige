...
(function (global) {
  ...
  function render(container) {
        ...
        '<div class="glass-card mb-24" id="fs-input-card">' +
          ...
          '<div class="fs-input-row">' +
            '<div class="fs-input-group">' +
              ...
              '<input type="number" id="fs-build-year" class="fs-input" value="' + currentYear + '" min="1900" max="2100" placeholder="如 2024">' +
            '</div>' +
            '<div class="fs-input-group">' +
            ...
            '<div class="fs-input-group">' +
              ...
              '<input type="number" id="fs-current-year" class="fs-input" value="' + currentYear + '" min="2020" max="2100">' +
            '</div>' +
          '</div>' +
          '<div class="fs-btn-row">' +
    ...
    // 自动排盘
    setTimeout(function () { doCalc(container); }, 200);
  }
  ...
  function initMountainSelects(container) {
    ...
    var mountainNames = {
      '壬': '壬(北偏西)', '子': '子(正北)', '癸': '癸(北偏东)',
      '丑': '丑(东北偏北)', '艮': '艮(东北)', '寅': '寅(东北偏东)',
      '甲': '甲(东偏北)', '卯': '卯(正东)', '乙': '乙(东偏南)',
      '辰': '辰(东南偏东)', '巽': '巽(东南)', '巳': '巳(东南偏南)',
      '丙': '丙(南偏东)', '午': '午(正南)', '丁': '丁(南偏西)',
      '未': '未(西南偏南)', '坤': '坤(西南)', '申': '申(西南偏西)',
      '庚': '庚(西偏南)', '酉': '酉(正西)', '辛': '辛(西偏北)',
      '戌': '戌(西北偏西)', '乾': '乾(西北)', '亥': '亥(西北偏北)'
    };
  ...
  function initBuilder(container) {
    ...
    ROOM_TYPES.forEach(function (room) {
      ...
      btn.innerHTML = '<span class="fs-room-icon">' + room.icon + '</span><span class="fs-room-name">' + room.name + '</span>';
    ...
    function selectRoom(room, btn) {
      selectedRoom = room;
      palette.querySelectorAll('.fs-room-btn').forEach(function (b) { b.classList.remove('selected'); });
    ...
    container.querySelectorAll('.fs-grid-size-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        container.querySelectorAll('.fs-grid-size-btn').forEach(function (b) { b.classList.remove('active'); });
        this.classList.add('active');
      });
    });
    ...
    function renderGrid() {
      ...
      cells.forEach(function (cell) {
        ...
        if (room) {
          ...
          cell.innerHTML = '<span class="fs-cell-room-icon">' + room.icon + '</span>' +
            '<span class="fs-cell-room-name">' + room.name + '</span>' +
            '<span class="fs-cell-palace-name">' + PALACE_SHORT[pi] + '</span>';
        } else {
          cell.innerHTML = '<span class="fs-cell-placeholder">+</span>';
        }
      });
    ...
    function doCalcWithCompass() {
      ...
      currentResult = global.FengshuiEngine.divine({ sitting: currentSitting, facing: currentFacing, buildYear: buildYear, currentYear: currentYr });
    ...
    function updateFloorPlanOverlay() {
      ...
      cells.forEach(function (cell) {
        ...
        if (palace) {
          var levelColor = palace.level === '大吉' || palace.level === '吉' ? 'rgba(90,138,90,0.15)' :
            palace.level === '中平' ? 'rgba(138,130,120,0.1)' : 'rgba(200,64,64,0.12)';
    ...
    container.querySelector('#fs-random-rooms-btn').addEventListener('click', function () {
      ...
      for (var i = 0; i < essential.length && i < totalCells; i++) {
        gridRooms[indices[i]] = ROOM_TYPES.find(function (r) { return r.id === essential[i]; });
      }
      ...
      for (var i = 0; i < extraCount && i + essential.length < totalCells; i++) {
        gridRooms[indices[i + essential.length]] = ROOM_TYPES.find(function (r) { return r.id === optional[i]; });
      }
      renderGrid();
      if (currentResult) updateFloorPlanOverlay();
    });
    ...
    updateCompassInfo();
    drawCompass();
    window.addEventListener('resize', function () { drawCompass(); });
  }
  ...
  function initUpload(container) {
    ...
    var gridX = 0, gridY = 0, gridW = 300, gridH = 300;
    var dragging = false, dragStartX = 0, dragStartY = 0, gridStartX = 0, gridStartY = 0;
    var currentResult = null;

    uploadZone.addEventListener('click', function (e) { if (e.target !== fileInput) fileInput.click(); });
    fileInput.addEventListener('change', function (e) { if (e.target.files && e.target.files[0]) loadImage(e.target.files[0]); });
    ...
    function resetGrid() {
      ...
      gridW = minDim * 0.75; gridH = gridW;
      gridX = (floorCanvas.width - gridW) / 2; gridY = (floorCanvas.height - gridH) / 2;
    }

    window.addEventListener('resize', function () {
      if (floorImg) { resetGrid(); drawFloorPlan(); }
    });
    ...
    floorCanvas.addEventListener('mousedown', function (e) {
      ...
      if (mx >= gridX && mx <= gridX + gridW && my >= gridY && my <= gridY + gridH) {
        dragging = true; dragStartX = mx; dragStartY = my; gridStartX = gridX; gridStartY = gridY;
    ...
    floorCanvas.addEventListener('mousemove', function (e) {
      ...
      var my = (e.clientY - rect.top) * scaleY;
      if (dragging) {
        gridX = gridStartX + (mx - dragStartX); gridY = gridStartY + (my - dragStartY);
        ...
        floorCanvas.style.cursor = (mx >= gridX && mx <= gridX + gridW && my >= gridY && my <= gridY + gridH) ? 'grab' : 'default';
      }
    });

    window.addEventListener('mouseup', function () { dragging = false; floorCanvas.style.cursor = 'grab'; });
    ...
    floorCanvas.addEventListener('wheel', function (e) {
      ...
      var newW = gridW * zoom, newH = gridH * zoom;
      if (newW < 40 || newH < 40 || newW > floorCanvas.width * 2 || newH > floorCanvas.height * 2) return;
      var ratioX = (mx - gridX) / gridW; var ratioY = (my - gridY) / gridH;
      gridX = mx - newW * ratioX; gridY = my - newH * ratioY; gridW = newW; gridH = newH;
      drawFloorPlan();
    });

    gridOpacitySlider.addEventListener('input', function () { drawFloorPlan(); });
    container.querySelector('#fs-reset-grid-btn').addEventListener('click', function () { resetGrid(); drawFloorPlan(); });
    toggleGridBtn.addEventListener('click', function () {
      gridVisible = !gridVisible; toggleGridBtn.textContent = gridVisible ? '隐藏网格' : '显示网格'; drawFloorPlan();
    });
    container.querySelector('#fs-remove-img-btn').addEventListener('click', function () {
      floorImg = null; uploadZone.classList.remove('has-image');
      container.querySelector('#fs-upload-placeholder').style.display = '';
      floorWrap.style.display = 'none'; floorControls.style.display = 'none'; fileInput.value = '';
    });

    function drawFloorPlan() {
      if (!floorImg) return;
      var cw = floorCanvas.width, ch = floorCanvas.height;
      ...
      for (var di = 0; di < displayIdx.length; di++) {
        ...
        var row = Math.floor(di / 3), col = di % 3;
        ...
        fctx.fillStyle = bgColor + (opacity * 0.5) + ')'; fctx.fillRect(cx, cy, cellW, cellH);
        fctx.strokeStyle = 'rgba(184,154,92,' + (opacity * 0.6) + ')'; fctx.lineWidth = 1.5; fctx.strokeRect(cx, cy, cellW, cellH);
        if (palace) {
          ...
          var levelColor = palace.level === '大吉' || palace.level === '吉' ? '#a8d8a8' : palace.level === '中平' ? '#a8b8c8' : '#d8a8a8';
          fctx.fillStyle = levelColor; fctx.fillText(palace.level + ' ' + palace.score, cx + cellW / 2, cy + cellH * 0.6);
          ...
          fctx.fillText('运' + palace.periodStar + ' 山' + palace.mountainStar + ' 向' + palace.facingStar + ' 年' + palace.annualStar, cx + cellW / 2, cy + cellH * 0.82);
        }
      }
      fctx.strokeStyle = 'rgba(184,154,92,' + (opacity * 0.8) + ')'; fctx.lineWidth = 2; fctx.strokeRect(gridX, gridY, gridW, gridH);
      var cornerSize = Math.min(cellW, cellH) * 0.12;
      fctx.fillStyle = 'rgba(184,154,92,' + (opacity * 0.6) + ')';
      [[gridX, gridY], [gridX + gridW - cornerSize, gridY], [gridX, gridY + gridH - cornerSize], [gridX + gridW - cornerSize, gridY + gridH - cornerSize]].forEach(function (pt) {
  ...
  function displayResult(container, result) {
    ...
    var levelColors = {
      '上吉': '#c8d8a8', '中吉': '#d8c8a8', '中平': '#a8b8c8', '凶': '#d8a8a8', '大凶': '#d84040'
    };
    ...
    container.querySelector('#fs-overview-info').textContent =
      '坐' + result.sitting + '向' + result.facing + ' | ' + result.buildYear + '年建（' + result.periodNum + '运）| ' + result.currentYear + '年流年飞星';
  ...
  function renderGrid(container, result) {
    ...
    for (var di = 0; di < displayIdx.length; di++) {
      ...
      var levelClass = 'fs-cell-' + (p.level === '大吉' || p.level === '吉' ? 'ji' : p.level === '中平' ? 'zhong' : 'xiong');
      html += '<div class="fs-cell ' + levelClass + '">' +
        '<div class="fs-cell-name">' + (p.name || '') + '</div>' +
        '<div class="fs-cell-stars">' +
          '<span class="fs-star-tag" style="background:' + getStarColor(p.periodStar, stars) + ';">运' + p.periodStar + '</span>' +
          '<span class="fs-star-tag" style="background:' + getStarColor(p.mountainStar, stars) + ';">山' + p.mountainStar + '</span>' +
          '<span class="fs-star-tag" style="background:' + getStarColor(p.facingStar, stars) + ';">向' + p.facingStar + '</span>' +
          '<span class="fs-star-tag" style="background:' + getStarColor(p.annualStar, stars) + ';">年' + p.annualStar + '</span>' +
        '</div>' +
        '<div class="fs-cell-level">' + p.level + ' (' + p.score + ')</div>' +
        '</div>';
  ...
  function renderBestWorst(container, result) {
    ...
    bestEl.innerHTML = '<h3 style="color:#c8d8a8;margin-bottom:12px;">✅ 最佳方位</h3>' +
      '<div style="font-size:1.1rem;font-weight:600;color:var(--gold-light);">' + (bp.name || '') + '</div>' +
      '<div style="color:var(--text-muted);margin:8px 0;">综合评分：' + bp.score + '（' + bp.level + '）</div>' +
      (bp.notes && bp.notes.length > 0 ? '<div style="color:#a8c8a8;">' + bp.notes.join('；') + '</div>' : '');

    worstEl.innerHTML = '<h3 style="color:#d8a8a8;margin-bottom:12px;">⚠️ 最差方位</h3>' +
      '<div style="font-size:1.1rem;font-weight:600;color:var(--gold-light);">' + (wp.name || '') + '</div>' +
      '<div style="color:var(--text-muted);margin:8px 0;">综合评分：' + wp.score + '（' + wp.level + '）</div>' +
  ...
  function renderTable(container, result) {
    ...
    var stars = (global.FengshuiEngine && global.FengshuiEngine.STARS) || {};
    var html = '<table class="fs-detail-table"><thead><tr>' +
    ...
    for (var i = 0; i < result.palaces.length; i++) {
      ...
      html += '<tr>' +
        ...
        '<td>' + (p.gua || '') + '</td>' +
        '<td>' + (p.wuxing || '') + '</td>' +
        '<td><span class="fs-star-dot" style="background:' + getStarColor(p.periodStar, stars) + ';"></span>' + p.periodStar + '</td>' +
        '<td><span class="fs-star-dot" style="background:' + getStarColor(p.mountainStar, stars) + ';"></span>' + p.mountainStar + '</td>' +
        '<td><span class="fs-star-dot" style="background:' + getStarColor(p.facingStar, stars) + ';"></span>' + p.facingStar + '</td>' +
        '<td><span class="fs-star-dot" style="background:' + getStarColor(p.annualStar, stars) + ';"></span>' + p.annualStar + '</td>' +
        '<td style="font-weight:600;">' + p.score + '</td>' +
        '<td style="color:' + (p.level === '大吉' || p.level === '吉' ? '#c8d8a8' : p.level === '中平' ? '#a8b8c8' : '#d8a8a8') + ';font-weight:600;">' + p.level + '</td>' +
        '<td style="font-size:0.8rem;color:var(--text-muted);">' + (allNotes.length > 0 ? allNotes.join('；') : '—') + '</td>' +
        '</tr>';
    }
    html += '</tbody></table>';
