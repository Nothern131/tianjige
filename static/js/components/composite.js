...
function renderCompositeComponent() {
  try {
    ...
    for (var i = 0; i < METHOD_CHECKS.length; i++) {
      ...
      methodHtml += '<label style="display:flex;align-items:center;gap:4px;padding:6px 10px;background:rgba(184,154,92,0.08);border-radius:6px;cursor:pointer;font-size:0.85rem;">' +
        '<input type="checkbox" name="compMethod" value="' + m.id + '"' + (m.checked ? ' checked' : '') + '>' +
        m.icon + ' ' + m.name + '</label>';
    }
    ...
    for (var j = 0; j < MASTER_OPTIONS.length; j++) {
      ...
      masterHtml += '<label style="display:flex;align-items:center;gap:4px;padding:6px 10px;background:rgba(184,154,92,0.08);border-radius:6px;cursor:pointer;font-size:0.85rem;">' +
        '<input type="checkbox" name="compMaster" value="' + ma.id + '"' + masterChecked + '>' +
        ma.emoji + ' ' + ma.name + '</label>';
    }
    ...
    container.innerHTML = '' +
      ...
      '<div class="glass-card mb-24" id="composite-input-card">' +
        ...
        '<div class="form-group" style="margin-top:12px;">' +
          ...
          '<div id="comp-method-checks" style="display:flex;flex-wrap:wrap;gap:6px;">' + methodHtml + '</div>' +
        ...
        '<div class="form-group" style="margin-top:12px;">' +
          ...
          '<div id="comp-master-checks" style="display:flex;flex-wrap:wrap;gap:6px;">' + masterHtml + '</div>' +
        '</div>' +
    ...
    errDiv.innerHTML = '<div class="error-icon">⚠️</div><div class="error-text">合参组件加载失败：' + (e.message || '未知错误') + '</div>';
    return errDiv;
  }
}
...
function handleCompositeSubmit(container) {
  try {
    ...
    var ENGINE_NAMES = {
      bazi: '八字', liuyao: '六爻', meihua: '梅花',
      qimen: '奇门', taiyi: '太乙', zhuge: '诸葛', zhougong: '解梦'
    };
    ...
    updateProgress(10, '启动引擎: ' + methods.map(function (m) { return ENGINE_NAMES[m] || m; }).join(', '));

    if (typeof CompositeEngine === 'undefined') {
      resultContainer.innerHTML = '<div class="error-container"><div class="error-icon">⚠️</div><div class="error-text">合参引擎未加载，请刷新页面重试。</div></div>';
      if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = '🔮 重新推演'; }
      if (progressBar) progressBar.classList.add('hidden');
      return;
    }

    CompositeEngine.run(methods, engineParams).then(function (data) {
      ...
      resultContainer.innerHTML = '<div class="error-container"><div class="error-icon">⚠️</div><div class="error-text">' + safeHtml(err.message || '计算失败') + '</div></div>';
      if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = '🔮 重新推演'; }
      if (progressBar) progressBar.classList.add('hidden');
    ...
    if (rc) rc.innerHTML = '<div class="error-container"><div class="error-icon">⚠️</div><div class="error-text">' + safeHtml(e.message || '未知错误') + '</div></div>';
    var sb = container.querySelector('#comp-submit-btn');
    if (sb) { sb.disabled = false; sb.textContent = '🔮 重新推演'; }
...
function formatAnalysisText(text) {
  ...
  return text.split('\n').map(function (line) {
    ...
    if (trimmed.indexOf('【') === 0) return '<strong style="color:var(--gold-light);">' + safeHtml(trimmed) + '</strong>';
...
function renderCompositeResult(container, data, question, methods, masterIds) {
  if (!data) {
    container.innerHTML = '<div class="error-container"><div class="error-icon">⚠️</div><div class="error-text">无有效结果</div></div>';
    return;
  }
  ...
  html += '' +
    '<div class="glass-card mb-24" style="text-align:center;">' +
      '<div style="font-size:0.9rem;color:var(--text-muted);margin-bottom:8px;">' + safeHtml(synthesis.consensus || '合参推演') + '</div>' +
      '<div style="font-family:var(--font-serif);font-size:3.5rem;font-weight:900;color:var(--gold-light);line-height:1;">' + score + '</div>' +
      '<div style="font-size:1.1rem;color:var(--gold);margin:8px 0;">' + stars + '</div>' +
      '<div style="font-size:0.9rem;color:var(--text-muted);">' + trendEmoji + ' ' + trendText + ' · ' + results.length + ' 门术数参与</div>' +
    '</div>';

  // --- 各术数明细 ---
  if (results.length > 0) {
    html += '<div class="glass-card mb-24"><h3 style="color:var(--gold-light);margin-bottom:12px;">📊 各术数独立推演</h3><div class="card-grid cols-1" style="gap:12px;">';
        ...
        '<div class="glass-card" style="padding:14px;background:rgba(255,255,255,0.02);">' +
          '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">' +
            '<div style="display:flex;align-items:center;gap:8px;">' +
              '<span style="font-size:1.2rem;">' + safeHtml(r.icon) + '</span>' +
              '<span style="font-family:var(--font-serif);font-weight:600;color:var(--gold);">' + safeHtml(r.name) + '</span>' +
            '</div>' +
            '<div style="text-align:right;">' +
              '<div style="font-size:1.1rem;font-weight:700;color:' + verdictColor + '">' + r.score + '分</div>' +
              '<div style="font-size:0.7rem;color:var(--text-muted);">' + starMini + ' ' + trendArr + '</div>' +
            '</div>' +
          '</div>' +
          '<div style="font-size:0.85rem;color:var(--text-secondary);line-height:1.6;">' + safeHtml(r.summary) + '</div>' +
          '<div style="margin-top:6px;font-size:0.75rem;color:var(--text-muted);">断语：' + safeHtml(r.verdict) + '</div>' +
          '<div style="margin-top:4px;font-size:0.72rem;color:var(--text-muted);">' +
            (r.keyInsights || []).map(function (k) { return '✦ ' + safeHtml(k); }).join(' ') +
          '</div>' +
        '</div>';
    }
    html += '</div></div>';
  ...
  if (synthesis.domainAnalysis) {
    ...
    html += '' +
      '<div class="glass-card mb-24">' +
        '<h3 style="color:var(--gold-light);margin-bottom:12px;">' + safeHtml(da.domainIcon) + ' 事域交叉分析：' + safeHtml(da.domainName) + '</h3>' +
        '<div class="verdict-block" style="margin:0 0 16px 0;"><div class="verdict-text">' + safeHtml(da.consensusText) + '</div></div>' +
        '<div style="display:flex;gap:16px;margin-bottom:12px;font-size:0.8rem;">' +
          '<span style="color:var(--success);">↑ 看好 ×' + (da.upCount || 0) + '</span>' +
          '<span style="color:var(--warning);">→ 平稳 ×' + (da.flatCount || 0) + '</span>' +
          '<span style="color:var(--danger);">↓ 谨慎 ×' + (da.downCount || 0) + '</span>' +
        '</div>';
    if (da.insights && da.insights.length > 0) {
      ...
      for (var di = 0; di < da.insights.length; di++) {
        html += '<div style="margin-bottom:6px;">' + safeHtml(da.insights[di].icon) + ' <strong>' + safeHtml(da.insights[di].method) + '</strong>：' + safeHtml(da.insights[di].insight) + '</div>';
      }
      html += '</div>';
    }
    html += '</div>';
      ...
      '<div class="glass-card mb-24">' +
        ...
        '<div style="display:flex;gap:16px;margin-bottom:16px;text-align:center;">' +
          '<div style="flex:1;background:rgba(184,154,92,0.06);border-radius:8px;padding:12px;">' +
            ...
            '<div style="font-size:1.6rem;font-weight:700;color:' + (ha.avgHarmony >= 60 ? '#c8d8a8' : '#e6d8a8') + ';">' + ha.avgHarmony + '%</div>' +
          '</div>' +
          '<div style="flex:1;background:rgba(184,154,92,0.06);border-radius:8px;padding:12px;">' +
            '<div style="font-size:0.75rem;color:var(--text-muted);">总体磨合度</div>' +
            '<div style="font-size:1.6rem;font-weight:700;color:' + (ha.avgFriction <= 40 ? '#c8d8a8' : '#d8a8a8') + ';">' + ha.avgFriction + '%</div>' +
          '</div>' +
        '</div>' +
        '<div class="analysis-content" style="white-space:normal;margin-bottom:16px;">' + formatAnalysisText(ha.analysis) + '</div>';

    // 视觉化融洽度条
    html += '<div style="margin-top:12px;">';
    for (var pi = 0; pi < ha.pairs.length; pi++) {
      var p = ha.pairs[pi];
      var harmonyColor = p.harmony >= 80 ? '#c8d8a8' : (p.harmony >= 60 ? '#e6d8a8' : (p.harmony >= 40 ? '#d8c8a8' : '#d8a8a8'));
      html += '' +
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;font-size:0.8rem;">' +
          '<span style="min-width:80px;text-align:right;">' + safeHtml(p.iconA) + ' ' + safeHtml(p.methodA) + '</span>' +
          '<span style="color:var(--text-muted);">↔</span>' +
          '<span style="min-width:80px;">' + safeHtml(p.iconB) + ' ' + safeHtml(p.methodB) + '</span>' +
          '<div style="flex:1;height:8px;background:rgba(255,255,255,0.04);border-radius:4px;overflow:hidden;">' +
            '<div style="height:100%;width:' + p.harmony + '%;background:' + harmonyColor + ';border-radius:4px;transition:width 0.6s ease;"></div>' +
          '</div>' +
          '<span style="min-width:55px;text-align:right;font-size:0.75rem;color:' + harmonyColor + ';">' + p.harmony + '% ' + p.harmonyLevel + '</span>' +
        '</div>';
    }
    html += '</div></div>';
  }
  ...
  html += '' +
    '<div class="glass-card mb-24">' +
      ...
      '<div class="analysis-content" style="white-space:normal;">' + formatAnalysisText(synthesis.narrative) + '</div>' +
      '<div style="margin-top:12px;font-size:0.8rem;color:var(--text-muted);">' +
        '置信度：<span style="color:' + (synthesis.confidence === 'high' ? 'var(--gold)' : 'var(--text-secondary)') + ';">' +
        (synthesis.confidence === 'high' ? '高 · 各术数结论一致' : (synthesis.confidence === 'medium' ? '中 · 存在部分分歧' : '低 · 数据不足')) +
        '</span> · 术数分歧度：' + (synthesis.spread || 0) + '分' +
      '</div>' +
    '</div>';
      ...
      html += '' +
        '<div style="margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid var(--border-subtle);">' +
          '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">' +
            '<span style="font-size:1.3rem;">' + safeHtml(mc.avatar) + '</span>' +
            '<span style="font-family:var(--font-serif);font-weight:600;color:var(--gold);">' + safeHtml(mc.name) + '</span>' +
            '<span style="font-size:0.75rem;color:var(--text-muted);">· ' + safeHtml(mc.era) + ' · ' + safeHtml(mc.title) + '</span>' +
          '</div>' +
          '<div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:6px;font-style:italic;">「' + safeHtml(mc.style) + '」</div>' +
          '<div style="font-size:0.85rem;color:var(--text-secondary);line-height:1.8;">' + formatAnalysisText(mc.commentary) + '</div>' +
        '</div>';
    }
    html += '</div>';
  }
  ...
  // --- 免责声明 ---
  html += '<div style="text-align:center;font-size:0.75rem;color:var(--text-muted);margin-top:20px;">多术数合参仅供参考，命由天定，运由己造。天机不可尽泄，留三分与造化。</div>';
  html += '</div>';
