...
(function (global) {
  ...
  function getWuxingDuality(relation) {
    return WUXING_DUALITY[relation] || {
      yang: '运势平稳，无大起大落',
      yin: '缺乏突破契机，易陷入平庸',
  ...
  function addBalance(mainText, score, options) {
    ...
    if (score >= 70 && counterSignals.length > 0) {
      parts.push('然而，' + counterSignals.map(function (s) {
        return s.label;
      }).join('，') + '，需加以留意。');
    }
    // 低分但有转机
    else if (score <= 40 && counterSignals.length > 0) {
      parts.push('不过，' + counterSignals.map(function (s) {
        return s.label;
      }).join('，') + '，仍有转圜余地。');
    }
    // 中等分数，正反两面都要展示
    else if (score >= 40 && score <= 60 && counterSignals.length > 0) {
      parts.push('需注意，' + counterSignals.map(function (s) {
        return s.label;
      }).join('，') + '，利弊并存，需仔细权衡。');
    }
  ...
  function generateFullDuality(params) {
    ...
    // 计算正负权重
    var posWeight = positives.reduce(function (sum, p) { return sum + (p.weight || 1); }, 0);
    var negWeight = negatives.reduce(function (sum, n) { return sum + (n.weight || 1); }, 0);
    ...
    var key = '';
    if (balanceRatio >= 1.5) {
      key = '吉势已成，关键在于"守"——守住优势不骄不躁，将好运转化为实际成果。同时警惕' +
        (negatives.length > 0 ? negatives[0].label : '潜在风险') + '，不可因顺境而失去警觉。';
    } else if (balanceRatio >= 0.7) {
      key = '吉凶交织，关键在于"择"——明辨利弊，在复杂的局面中找到最有利的方向。' +
        '强化' + (positives.length > 0 ? positives[0].label : '优势面') +
        '，规避' + (negatives.length > 0 ? negatives[0].label : '风险面') + '。';
    } else {
      key = '形势不利，但关键在于"忍"与"变"——忍一时之困，变不利为有利。' +
        '低谷期宜' + (positives.length > 0 ? '发挥' + positives[0].label + '的优势' : '韬光养晦') +
        '，等待转机到来。';
    }
    ...
    if (overallScore >= 70) {
      ...
      advice = '运势低迷，宜以守为攻：1) 减少不必要的大动作；2) 关注身心健康，保持良好状态；3) 利用低谷期学习充电，为未来蓄力。';
    }
