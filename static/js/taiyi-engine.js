...
(function (global) {
  ...
  function calcSanji(jiNian) {
    ...
    return '君基入' + jun + '宫（' + GONG_DIR[jun] + '），臣基入' + chen + '宫（' + GONG_DIR[chen] + '），民基入' + min + '宫（' + GONG_DIR[min] + '）';
  }
  ...
  function generateInterpretation(taiyiGong, gods, wufu, sanji, sishen, juShu) {
    ...
    // 四神解读
    if (sishen) {
      lines.push('天乙' + sishen.tianyi.position + '，地乙' + sishen.diyi.position + '，直符' + sishen.zhifu.position + '，四神' + sishen.sishen.position + '。四神护佑，吉凶互见。');
    }
