...
(function (global) {
  ...
  function interpretPoemLine(line, domainKey) {
    // 事域中文名（用于解读前缀）
    var domainLabel = domainKey === 'career' ? '事业' : domainKey === 'love' ? '感情' : domainKey === 'wealth' ? '财运' : domainKey === 'health' ? '健康' : domainKey === 'travel' ? '出行' : domainKey === 'study' ? '学业' : '所问之事';
    var trimmed = line.trim();
    var len = trimmed.length;

    // ===== 意象词库：每个事域对应一组意象及其解读（命中时把原文嵌入解读） =====
    var interpretations = {
      career: {
        龙: ['龙', '贵人之象', '事业上有贵人相助或升迁之机，宜主动争取'],
        虎: ['虎', '威猛之象', '事业上需展现魄力与决断，但不可过于强势'],
        凤: ['凤', '鸣朝阳', '事业上将有亮眼表现，适合争取晋升'],
        水: ['水', '流动与智慧', '事业上宜灵活应变、顺势而为，忌固守成规'],
        山: ['山', '稳固', '事业根基扎实，但需防固步自封，拓展视野'],
        风: ['风', '变动', '事业上可能有职务或方向调整，宜随机应变'],
        云: ['云', '遮蔽与机遇', '事业上可能有不确定因素，需等待云开见日'],
        日: ['日', '光明', '事业前景明朗，宜积极进取，把握上升期'],
        月: ['月', '阴柔沉静', '事业上宜以柔克刚，韬光养晦'],
        花: ['花', '繁荣与生机', '花开花落有时，事业需把握时机、不可拖延'],
        春: ['春', '新生与萌动', '事业有新的开始或转机，宜主动出击'],
        秋: ['秋', '收获与收敛', '事业成果将现，但需防得意忘形、留余地'],
        金: ['金', '刚毅与财权', '事业上适合争取管理岗位或决策权'],
        玉: ['玉', '温润与品质', '事业上宜以德服人，口碑积累带来长期回报'],
        剑: ['剑', '锋芒与决断', '事业上需果断处理问题，但不宜锋芒太露'],
        桥: ['桥', '连接与过渡', '事业上可能有岗位转换或跨部门合作机会'],
        路: ['路', '前行与方向', '事业上宜明确目标、坚定步伐，忌犹豫'],
        星: ['星', '指引与希望', '事业上有新的方向指引，宜开阔视野'],
      },
      love: {
        龙: ['龙', '贵人引荐', '感情中可能有缘分降临，宜多社交'],
        凤: ['凤', '良缘之象', '单身者桃花运旺，有伴者感情升温'],
        水: ['水', '柔情与包容', '感情中需多些温柔体贴，顺其自然'],
        花: ['花', '桃花与心动', '感情有新机遇，需分辨正缘与烂桃花'],
        月: ['月', '阴晴圆缺', '感情中需接受不完美，珍惜当下、坦诚沟通'],
        春: ['春', '萌动与新生', '感情有新的开始，宜主动表达心意'],
        金: ['金', '坚定与承诺', '感情中需明确态度、信守承诺，忌三心二意'],
        玉: ['玉', '温润与长久', '感情宜以真心相待，细水长流'],
        鸟: ['鸟', '自由与迁徙', '感情中可能有异地或分离之象，需用心维系'],
        影: ['影', '虚幻与暧昧', '感情中可能有不确定因素，宜擦亮眼睛'],
        缘: ['缘', '命中注定', '感情有善缘，宜把握、主动经营'],
        心: ['心', '真诚', '感情贵在真心相待，忌猜疑与冷漠'],
      },
      wealth: {
        金: ['金', '财富', '财运上升，但需防贪多嚼不烂，量力而行'],
        水: ['水', '流动', '钱财来去较快，需做好理财规划、留有余粮'],
        山: ['山', '积蓄与稳健', '财运稳健，适合长期投资、不宜短线炒作'],
        秋: ['秋', '收获', '之前的投资将回报，宜落袋为安'],
        宝: ['宝', '珍贵与机遇', '有意外之财或贵重机遇，宜抓住、不犹豫'],
        田: ['田', '根基与产业', '财运宜稳守，适合购置不动产或长期持有'],
        仓: ['仓', '积蓄与储备', '财运宜先储蓄后消费，量入为出'],
        商: ['商', '交易与流通', '财运宜主动出击，拓展客源与渠道'],
        利: ['利', '收益与回报', '财运向好，但需警惕过劳与贪心'],
        债: ['债', '压力与拖累', '财运可能有负债或资金紧张，宜谨慎借贷'],
      },
      health: {
        水: ['水', '肾与血液', '健康上需关注肾脏、血液循环，适当补水、避免熬夜'],
        火: ['火', '心脏与血压', '健康上需关注心血管，避免情绪过激、饮食过辣'],
        木: ['木', '肝胆', '健康上需疏肝解郁，避免情绪压抑，多亲近自然'],
        金: ['金', '肺与呼吸', '健康上需关注呼吸系统，避免烟尘、戒烟'],
        土: ['土', '脾胃', '健康上需调养脾胃，饮食规律、忌生冷油腻'],
        气: ['气', '生命力', '健康上需调息养气，适度运动、充足睡眠'],
        眠: ['眠', '睡眠', '健康上需改善睡眠质量，忌熬夜、睡前屏弃电子'],
        骨: ['骨', '筋骨', '健康上需锻炼筋骨，适当运动、防跌摔'],
      },
      travel: {
        舟: ['舟', '远行', '出行顺畅，适合远途旅行或出差，宜备好行囊'],
        车: ['车', '奔波与快速', '出行节奏较快，注意交通安全、防疲劳驾驶'],
        路: ['路', '前行', '出行顺利，但可能有绕路或延误，宜预留时间'],
        山: ['山', '阻隔', '出行可能遇阻或天气不佳，宜改期或备选方案'],
        风: ['风', '变化', '出行计划可能有变动，宜保持灵活'],
        云: ['云', '漂泊与不定', '出行可能有临时取消或改签，宜多备预案'],
        水: ['水', '流动', '出行宜近不宜远，避免水灾或潮湿地区'],
        驿: ['驿', '中途停留', '出行可能有中途歇脚或转机，不必赶'],
      },
      study: {
        书: ['书', '学问', '学业上有收获，宜多读书、勤复习，忌拖延'],
        笔: ['笔', '表达与创作', '学业上适合写论文或考试发挥，宜充分准备'],
        窗: ['窗', '专注与静修', '学业上需排除干扰、静心学习'],
        墨: ['墨', '沉淀与积累', '学业上需厚积薄发，忌急于求成'],
        灯: ['灯', '指引与启发', '学业上有老师或贵人指点迷津，宜虚心请教'],
        琴: ['琴', '韵律与节奏', '学业上宜按部就班、劳逸结合，忌熬夜赶工'],
        经: ['经', '经典与基础', '学业上需夯实基础、多读经典，忌好高骛远'],
        学: ['学', '进步', '学业向上，宜保持热情与专注'],
      },
    };
    var domainInterp = interpretations[domainKey] || interpretations['career'];

    // ===== 多意象匹配：把命中意象的原文嵌入解读，保证每句解读带签文印记 =====
    var matched = [];
    for (var key in domainInterp) {
      if (matched.length >= 3) break;
      if (line.indexOf(key) >= 0) {
        var entry = domainInterp[key];
        // 嵌入原文：句中"X"字主"Y"，Z
        matched.push('句中"' + key + '"字主' + entry[1] + '，' + entry[2]);
      }
    }
    if (matched.length > 0) {
      // 按命中的意象数量差异化收尾，避免多句雷同
      if (matched.length === 1) {
        return matched[0] + '。' + domainLabel + '上此句提示较为明确，当顺势而为。';
      } else if (matched.length === 2) {
        return matched[0] + '；' + matched[1] + '。' + domainLabel + '上两象并见，需权衡取舍，取利避弊。';
      } else {
        return matched[0] + '；' + matched[1] + '；' + matched[2] + '。' + domainLabel + '上多重意象交织，宜综合研判，择其善者而从之。';
      }
    }

    // ===== 无意象命中时的差异化 fallback：根据字数区间 + 具体字符生成不同措辞 =====
    // 提取句中出现的汉字（去重，用于差异化措辞）
    var chars = [];
    for (var ci = 0; ci < trimmed.length; ci++) {
      var c = trimmed.charAt(ci);
      if (c >= '\u4e00' && c <= '\u9fff' && chars.indexOf(c) < 0) chars.push(c);
    }
    // 按字数区间 + 首字符差异化，确保不同签文不撞句
    var firstChar = chars[0] || '';
    var hintChars = chars.slice(0, 2).join('、') || '此字';

    if (len <= 4) {
      return '「' + trimmed + '」四字短促，' + domainLabel + '上提示局势明朗直接，宜直面无迂回，抓住「' + hintChars + '」所示关键。';
    } else if (len <= 8) {
      return '「' + trimmed + '」句意凝练，' + domainLabel + '上主' + (firstChar ? '「' + firstChar + '」之象' : '简洁之机') + '，宜简明决断，勿过虑。';
    } else if (len > 20) {
      return '「' + trimmed + '」句长意象丰富，' + domainLabel + '上提示事态较复杂，' + (chars.length >= 4 ? '多象交织（' + chars.slice(0, 4).join('、') + '），需综合权衡' : '需仔细甄别各意象') + '，不可偏听偏信。';
    } else if ('吉凶得失成败进退兴衰盈亏' + '福祸利害'.split('').some(function (c) { return trimmed.indexOf(c) !== -1; })) {
      // 含吉凶暗示字 → 提取具体暗示字做差异化
      var hint = '吉凶得失成败进退兴衰盈亏福祸利害'.split('').filter(function (c) { return trimmed.indexOf(c) !== -1; })[0] || '吉凶';
      return '此句含"' + hint + '"之暗示，' + domainLabel + '上需结合签等判断：' + (hint === '吉' || hint === '成' || hint === '得' || hint === '盈' ? '吉兆已现，宜顺势把握' : '警示在侧，宜守正待时') + '，' + (hint === '凶' || hint === '败' || hint === '失' || hint === '亏' ? '慎防不测' : '稳中求进') + '。';
    } else if (len <= 12) {
      return '「' + trimmed + '」句法平和，' + domainLabel + '上提示当前状态平稳，「' + hintChars + '」之象宜保持现状、伺机而动。';
    } else {
      return '「' + trimmed + '」句意连贯，' + domainLabel + '上无明显转折，宜按部就班推进，关注「' + hintChars + '」所示细节，循序而不冒进。';
    }
  }
